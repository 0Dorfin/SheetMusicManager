...
; PROJECT_DIR:      Path to sub-project directory
; ICON:             Path to application icon
; LICENSE:          Path to application license
...
; TESSDATA_SOURCE   Tessdata source folder (to be read from on host machine)
!ifndef SUFFIX
  !define SUFFIX ""
!endif
...
Section "Hauptgruppe" SEC01
  ...
  ; Pre-populate a few Tesseract language files
  ; Source is the Audiveris dev/tessdata folder on host machine
  ; Target is a tessdata folder in user config folder on target machine
  SetOutPath "$AppData\AudiverisLtd\audiveris\config\tessdata"
  SetOverwrite on
  File /r "${TESSDATA_SOURCE}\tessdata\*.traineddata"
